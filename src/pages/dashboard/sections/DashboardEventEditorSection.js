...
export function DashboardEventEditorSection({ events, eventsLoading, eventsError, onReload }) {
    const { calendarStatus, conferencingStatus, getCalendarProviderStatus, getConferencingProviderStatus, hasConferencingCapability, } = useIntegrationState();
    ...
    const supportsConferencingCapabilities = hasConferencingCapability("GOOGLE_MEET")
    ...
    const effectiveConferenceOptions = conferenceOptions.filter((opt) => {
        if (supportsConferencingCapabilities && !hasConferencingCapability(toConferencingProviderEnum(opt.value)))
            return false;

...
import { hasConsumerMicrosoftConnection, hasConferencingProviderCapability, isConferencingCapabilityMapPopulated, isTeamsDisabledByRuntimeCapability, toCapabilityAwareUnsupportedMessage, unsupportedCapabilityMessage, } from "@/lib/conferencingCapabilities";
...
export function DashboardEventEditorSection({ events, eventsLoading, eventsError, onReload }) {
    const { calendarStatus, calendarConnections, conferencingStatus, conferencingRuntime, conferencingCapabilities, getCalendarProviderStatus, getConferencingProviderStatus, } = useIntegrationState();
    ...
    const isConferenceOptionEnabled = (value) => {
        if (value === "microsoft_teams" && teamsDisabledByRuntime)
            return false;
        return true;
    };
    const selectedConferenceProviderAvailable = effectiveConferenceOptions.some((option) => option.value === conferencingProvider && isConferenceOptionEnabled(option.value));

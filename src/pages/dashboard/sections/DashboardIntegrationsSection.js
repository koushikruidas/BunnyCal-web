...
import { isConferencingCapabilityMapPopulated, isNativeTeamsSupported, unsupportedCapabilityMessage } from "@/lib/conferencingCapabilities";
...
export function DashboardIntegrationsSection({ banner, integrationsError, clearBanner, integrationsLoading, refreshStatus, pendingAction, calendarStatus, calendarConnections, conferencingStatus, calendarCapabilities, conferencingCapabilities, getCalendarProviderStatus, getConferencingProviderStatus, onRequestDisconnect, onConnectCalendar, onConnectConferencing, }) {
    ...
    const conferencingProviders = Array.from(new Set([
        ...
        if (provider !== "microsoft_teams")
            return true;
        if (!isConferencingCapabilityMapPopulated(conferencingCapabilities))
            return true;
        return isNativeTeamsSupported(conferencingCapabilities);
    });
    const teamsHiddenByCapability = isConferencingCapabilityMapPopulated(conferencingCapabilities)
        && !isNativeTeamsSupported(conferencingCapabilities);

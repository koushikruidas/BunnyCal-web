...
export function getIntentFromSearch(search) {
    const q = new URLSearchParams(search);
...
export function buildSignInUrl(intent = { mode: "APP_LOGIN" }) {
    if (intent.mode === "APP_LOGIN")
        return "/sign-in?mode=APP_LOGIN";
...
export function resolvePostLoginPath(intent) {
    if (!intent)
        return "/dashboard";

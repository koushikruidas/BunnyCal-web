...
export function useBookingActions(hostKind = "authenticated-host") {
    ...
    const requestHold = useCallback(async () => {
        ...
        try {
            const hold = await bookingResolver.holdSlot(ctx.username, ctx.eventTypeSlug, {
    ...
    const reschedule = useCallback(async () => {
        if (ctx.loading)
            return false;
        const bookingId = ctx.confirmation?.bookingId || ctx.hold?.bookingId;
        if (!bookingId || !ctx.selectedSlot || !ctx.username || !ctx.eventTypeSlug)
            return false;
        try {
            const token = ctx.confirmation?.manageToken?.trim() || undefined;
